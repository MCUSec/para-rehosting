...
int MQTT::Client<Network, Timer, a, b>::sendPacket(int length, Timer& timer)
{
    int rc = FAILURE,
        sent = 0;
    // debug_log("sendPacket 1",sizeof("sendPacket 1"));
    while (sent < length)
    {
        debug_log("sendPacket 2",sizeof("sendPacket 2"));
        rc = ipstack.write(&sendbuf[sent], length - sent, timer.left_ms());
        debug_log("sendPacket 3",sizeof("sendPacket 3"));
        if (rc < 0){  // there was an error writing the data
            // debug_log("sendPacket 4",sizeof("sendPacket 4"));
            break;
        }
        sent += rc;
        timer.expired();
        if (timer.expired()){ // only check expiry after at least one attempt to write
            debug_log("sendPacket 5",sizeof("sendPacket 5"));
            break;
        }
    }
    if (sent == length)
    {
        debug_log("sendPacket 6",sizeof("sendPacket 6"));
        if (this->keepAliveInterval > 0)
            last_sent.countdown(this->keepAliveInterval); // record the fact that we have successfully sent the packet
        rc = SUCCESS;
    }
    else{
        debug_log("sendPacket 7",sizeof("sendPacket 7"));
        rc = FAILURE;
    }
