...
osEventFlagsId_t osEventFlagsNew (const osEventFlagsAttr_t *attr) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    ef_id = svcRtxEventFlagsNew(attr);
    port_svc_call_exit((uint32_t*)ef_id);
  }
  return ef_id;
}
...
uint32_t osEventFlagsSet (osEventFlagsId_t ef_id, uint32_t flags) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    event_flags =  svcRtxEventFlagsSet(ef_id, flags);
    port_svc_call_exit((uint32_t*)&event_flags);
  }
  return event_flags;
}
...
uint32_t osEventFlagsWait (osEventFlagsId_t ef_id, uint32_t flags, uint32_t options, uint32_t timeout) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    event_flags = svcRtxEventFlagsWait(ef_id, flags, options, timeout);
    port_svc_call_exit((uint32_t*)&event_flags);
  }
  return event_flags;
}
...
osStatus_t osEventFlagsDelete (osEventFlagsId_t ef_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxEventFlagsDelete(ef_id);
