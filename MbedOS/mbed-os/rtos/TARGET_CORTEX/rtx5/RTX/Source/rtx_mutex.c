...
osMutexId_t osMutexNew (const osMutexAttr_t *attr) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    mutex_id = svcRtxMutexNew(attr);
    port_svc_call_exit((uint32_t*)mutex_id);
  }
  return mutex_id;
}
...
osStatus_t osMutexAcquire (osMutexId_t mutex_id, uint32_t timeout) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMutexAcquire(mutex_id, timeout);
    // printf("osMutexAcquire:status=");
...
osStatus_t osMutexRelease (osMutexId_t mutex_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMutexRelease(mutex_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osThreadId_t osMutexGetOwner (osMutexId_t mutex_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    thread = svcRtxMutexGetOwner(mutex_id);
    port_svc_call_exit((uint32_t*)thread);
  }
  return thread;
}
...
osStatus_t osMutexDelete (osMutexId_t mutex_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMutexDelete(mutex_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
