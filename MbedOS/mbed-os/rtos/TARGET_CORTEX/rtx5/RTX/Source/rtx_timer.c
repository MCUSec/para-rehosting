...
osTimerId_t osTimerNew (osTimerFunc_t func, osTimerType_t type, void *argument, const osTimerAttr_t *attr) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    timer_id = svcRtxTimerNew(func, type, argument, attr);
    port_svc_call_exit((uint32_t*)timer_id);
  }
  return timer_id;
}
...
osStatus_t osTimerStart (osTimerId_t timer_id, uint32_t ticks) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxTimerStart(timer_id, ticks);
...
osStatus_t osTimerStop (osTimerId_t timer_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxTimerStop(timer_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
uint32_t osTimerIsRunning (osTimerId_t timer_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    is_running = svcRtxTimerIsRunning(timer_id);
    port_svc_call_exit((uint32_t*)&is_running);
  }
  return is_running;
}
...
osStatus_t osTimerDelete (osTimerId_t timer_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxTimerDelete(timer_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
