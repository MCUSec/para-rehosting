  ...
  if (msg != NULL) {
    ...
    if (timeout != 0U) {
      ...
      if (osRtxThreadWaitEnter(osRtxThreadWaitingMessageGet, timeout)) {; 
        ...
        //lint -e{923} -e{9078} "cast from unsigned int to pointer"
        // lwq I don't know where use it now, leave it for later port
        // reg = (uint32_t *)(__get_PSP());
        // //lint -e{923} -e{9078} "cast from pointer to unsigned int"
        // reg[2] = (uint32_t)msg_ptr;
        // //lint -e{923} -e{9078} "cast from pointer to unsigned int"
...
osMessageQueueId_t osMessageQueueNew (uint32_t msg_count, uint32_t msg_size, const osMessageQueueAttr_t *attr) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    mq_id = svcRtxMessageQueueNew(msg_count, msg_size, attr);
    port_svc_call_exit((uint32_t*)mq_id);
  }
  return mq_id;
}
...
osStatus_t osMessageQueuePut (osMessageQueueId_t mq_id, const void *msg_ptr, uint8_t msg_prio, uint32_t timeout) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMessageQueuePut(mq_id, msg_ptr, msg_prio, timeout);
...
osStatus_t osMessageQueueGet (osMessageQueueId_t mq_id, void *msg_ptr, uint8_t *msg_prio, uint32_t timeout) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMessageQueueGet(mq_id, msg_ptr, msg_prio, timeout);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
uint32_t osMessageQueueGetCapacity (osMessageQueueId_t mq_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    capacity = svcRtxMessageQueueGetCapacity(mq_id);
    port_svc_call_exit((uint32_t*)&capacity);
  }
  return capacity;
}
...
uint32_t osMessageQueueGetMsgSize (osMessageQueueId_t mq_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    msg_size = svcRtxMessageQueueGetMsgSize(mq_id);
    port_svc_call_exit((uint32_t*)&msg_size);
  }
  return msg_size;
}
...
uint32_t osMessageQueueGetSpace (osMessageQueueId_t mq_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    space = svcRtxMessageQueueGetSpace(mq_id);
    port_svc_call_exit((uint32_t*)&space);
  }
  return space;
}
...
osStatus_t osMessageQueueReset (osMessageQueueId_t mq_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMessageQueueReset(mq_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osStatus_t osMessageQueueDelete (osMessageQueueId_t mq_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxMessageQueueDelete(mq_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
