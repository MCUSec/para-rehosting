...
osSemaphoreId_t osSemaphoreNew (uint32_t max_count, uint32_t initial_count, const osSemaphoreAttr_t *attr) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    semaphore_id = svcRtxSemaphoreNew(max_count, initial_count, attr);
    port_svc_call_exit((uint32_t*)semaphore_id);
  }
  return semaphore_id;
}
...
osStatus_t osSemaphoreAcquire (osSemaphoreId_t semaphore_id, uint32_t timeout) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxSemaphoreAcquire(semaphore_id, timeout);
...
osStatus_t osSemaphoreRelease (osSemaphoreId_t semaphore_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxSemaphoreRelease(semaphore_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osStatus_t osSemaphoreDelete (osSemaphoreId_t semaphore_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxSemaphoreDelete(semaphore_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
