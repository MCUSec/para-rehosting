...
void osRtxThreadListPut (os_object_t *object, os_thread_t *thread) {
  ...
  while ((next != NULL) && (next->priority >= priority)) {
    // printf("osRtxThreadListPut loop\n");
    prev = next;
...
uint32_t *osRtxThreadRegPtr (const os_thread_t *thread) {
  ...
  return PortGetRegs((void*)thread);
}
...
void osRtxThreadWaitExit (os_thread_t *thread, uint32_t ret_val, bool_t dispatch) {
  ...
  // reg[0] = ret_val;
  *reg[0] = ret_val;
  // printf("osRtxThreadWaitExit:reg[0]=%x,reg[0] value=%u\n",reg[0],*reg[0]);
  // in arm board regs[0] is memory of R0, so just give it the new value;
  // but in port, we only could store the pointer to the memory, so we need to give the new value to the origin memory
...
static osThreadId_t svcRtxThreadNew (osThreadFunc_t func, void *argument, const osThreadAttr_t *attr) {
  ...
  if ((stack_size != 0U) && (((stack_size & 7U) != 0U) || (stack_size < (64U + 8U)))) {
    EvrRtxThreadError(NULL, osRtxErrorInvalidThreadStack);
    //lint -e{904} "Return statement before end of function" [MISRA Note 1]svcRtxThreadNew
    return NULL;
  }
  ...
  if (thread != NULL) {
  ...
  #endif
  #endif
    // lwq no stack initialize needed
    // Initialize stack
    ...
    // Linux Port
    MbedOS_FE_svcRtxThreadNew(thread, func, argument);
...
osThreadId_t osThreadNew (osThreadFunc_t func, void *argument, const osThreadAttr_t *attr) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    thread_id = svcRtxThreadNew(func, argument, attr);
    port_svc_call_exit((uint32_t*)thread_id);
...
const char *osThreadGetName (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_exit((uint32_t*)name);
  }
  return name;
}
...
osThreadState_t osThreadGetState (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    state = svcRtxThreadGetState(thread_id);
...
uint32_t osThreadGetStackSize (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    stack_size = svcRtxThreadGetStackSize(thread_id);
    port_svc_call_exit((uint32_t*)&stack_size);
  }
  return stack_size;
}
...
uint32_t osThreadGetStackSpace (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    stack_space = svcRtxThreadGetStackSpace(thread_id);
    port_svc_call_exit((uint32_t*)&stack_space);
  }
  return stack_space;
}
...
osStatus_t osThreadSetPriority (osThreadId_t thread_id, osPriority_t priority) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxThreadSetPriority(thread_id, priority);
...
osPriority_t osThreadGetPriority (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    priority = svcRtxThreadGetPriority(thread_id);
    port_svc_call_exit((uint32_t*)&priority);
  }
  return priority;
}
...
osStatus_t osThreadSuspend (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxThreadSuspend(thread_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osStatus_t osThreadResume (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxThreadResume(thread_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osStatus_t osThreadDetach (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxThreadDetach(thread_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osStatus_t osThreadJoin (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxThreadJoin(thread_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
osStatus_t osThreadTerminate (osThreadId_t thread_id) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    status = svcRtxThreadTerminate(thread_id);
    port_svc_call_exit((uint32_t*)&status);
  }
  return status;
}
...
uint32_t osThreadEnumerate (osThreadId_t *thread_array, uint32_t array_items) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    count = svcRtxThreadEnumerate(thread_array,array_items);
    port_svc_call_exit((uint32_t*)&count);
  }
  return count;
}
...
uint32_t osThreadFlagsSet (osThreadId_t thread_id, uint32_t flags) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    thread_flags = svcRtxThreadFlagsSet(thread_id, flags);
    port_svc_call_exit((uint32_t*)&thread_flags);
  }
  return thread_flags;
}
...
uint32_t osThreadFlagsClear (uint32_t flags) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    thread_flags = svcRtxThreadFlagsClear(flags);
    port_svc_call_exit((uint32_t*)&thread_flags);
  }
  return thread_flags;
}
...
uint32_t osThreadFlagsGet (void) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    thread_flags = svcRtxThreadFlagsGet();
    port_svc_call_exit((uint32_t*)&thread_flags);
  }
  return thread_flags;
}
...
uint32_t osThreadFlagsWait (uint32_t flags, uint32_t options, uint32_t timeout) {
  ...
  if (IsIrqMode() || IsIrqMasked()) {
    ...
    port_svc_call_enter();
    thread_flags = svcRtxThreadFlagsWait(flags, options, timeout);
    port_svc_call_exit((uint32_t*)&thread_flags);
  }
  return thread_flags;
}
